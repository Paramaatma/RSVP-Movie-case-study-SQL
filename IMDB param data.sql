...
select count(*) from  director_mapping; -- total number of rows in director_mapping

select count(*) from genre ; -- total number of rows in genre

select count(*) from  movie; -- total number of rows in movie

select count(*) from  names; -- total number of rows in names

select count(*) from  ratings; -- total number of rows in ratings

select count(*) from role_mapping ; -- total number of rows in role_mapping

-- Q2. Which columns in the movie table have null values?
-- Type your code below:

select count(*) from movie where country is null ; -- 20 null values

select count(*) from movie where worlwide_gross_income is null ; -- 3724 null vlaues

select count(*) from movie where languages is null ; -- 194 null values

select count(*) from movie where production_company is null ; -- 528 null values
...
select year, country_name, count(country_name) as number_of_movies_produced from country_code
where year = 2019 and country_name = 'India' or country_name = 'USA'
...
SELECT     m.title AS title,
           ...
           rank() over(ORDER BY round(avg(r.avg_rating),4) DESC) AS movie_rank
FROM       movie m
...
WHERE
    year = 2017
        ...
        AND m.country LIKE '%USA' AND m.country LIKE 'USA%' AND m.country LIKE '%USA%'
...
(
SELECT 
    m.title AS title, m.year AS year, median_rating
FROM
    movie m
        INNER JOIN
    ratings r ON m.id = r.movie_id
...
SELECT 
    ...
    median_rating AS median_rating_given
FROM
    released;
...
SELECT 
    n.name AS director_name, COUNT(g.movie_id) AS movie_count
FROM
    names AS n
        INNER JOIN
...
FROM
    ...
    names AS nm ON rm.name_id = nm.id
WHERE
    category = 'actress' AND country = 'india' and languages in ('Hindi')
GROUP BY name
